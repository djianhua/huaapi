...
import com.niuma.huaapi.facade.InterfaceInfoSearchFacade;
...
public class InterfaceInfoController {
    ...
    public BaseResponse<Object> invokeInterfaceInfo(@RequestBody InterfaceInfoInvokeRequest interfaceInfoInvokeRequest, HttpServletRequest request) {
        ...
        // 动态调用接口
        Object res = invokeInterfaceInfo(HuaApiClient.class, interfaceInfo.getMethodName(), userRequestParams, accessKey, secretKey);
        if (res == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR);
        }
    ...
    private Object invokeInterfaceInfo(Class<?> clientClazz, String methodName, String userRequestParams,
        ...
        try {
            // 1. 获取构造器，参数为ak,sk
            Constructor<?> huaApiClientConstructor = clientClazz.getConstructor(String.class, String.class);
            // 2. 构造出客户端
            HuaApiClient huaApiClient = (HuaApiClient) huaApiClientConstructor.newInstance(accessKey, secretKey);
            ...
            for (Method method : methods) {
                if (method.getName().equals(methodName)) {
                    ...
                    if (parameterTypes.length == 0) {
                        // 如果没有参数，直接调用
                        return method.invoke(huaApiClient);
                    }
                    Gson gson = new Gson();
                    // 构造参数
                    Object parameter = gson.fromJson(userRequestParams, parameterTypes[0]);
                    return method.invoke(huaApiClient, parameter);
                }
            }
            return null;
        } catch (Exception e) {
    ...
    public void getSdk(HttpServletResponse response) throws IOException {
        // 获取要下载的文件
        org.springframework.core.io.Resource resource = new ClassPathResource("huaapi-client-sdk-0.0.1.jar");
