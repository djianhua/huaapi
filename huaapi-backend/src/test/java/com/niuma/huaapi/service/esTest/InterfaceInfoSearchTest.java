...
class InterfaceInfoSearchTest {
    ...
    void testMatch() throws IOException {
        ...
        SearchRequest request = new SearchRequest("interface_info");
        // 2.准备请求参数
        request.source().query(QueryBuilders.matchQuery("all", "诗词"));
        ...
        */
        request.source().query(
                QueryBuilders.boolQuery()
                        .must(QueryBuilders.termQuery("method", "GET"))
                        .filter(QueryBuilders.rangeQuery("id").lte(45))
        );
        // 3.发送请求，得到响应
        SearchResponse response = client.search(request, RequestOptions.DEFAULT);
        // 4.结果解析
...
//        request.source().from((page - 1) * size).size(size);
    ...
    void testHighlight() throws IOException {
        ...
        request.source().query(QueryBuilders.matchQuery("all", "诗词"));
        // 2.2.高亮
        request.source().highlighter(new HighlightBuilder().field("description").requireFieldMatch(false));
        // 3.发送请求，得到响应
        SearchResponse response = client.search(request, RequestOptions.DEFAULT);
        // 4.结果解析
        handleResponse(response);
    ...
    private void handleResponse(SearchResponse response) {
        ...
        for (SearchHit hit : hits) {
            ...
            // 4.7.打印
            System.out.println(interfaceInfoDoc);
        }
    }
