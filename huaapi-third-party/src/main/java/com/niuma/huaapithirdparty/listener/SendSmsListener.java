package com.niuma.huaapithirdparty.listener;
...
import com.niuma.huaapithirdparty.utils.SendEmailUtils;
import com.niuma.huaapithirdparty.utils.SendSmsUtils;
...
import static com.niuma.huaapicommon.constant.RabbitMqConstant.SMS_DELAY_QUEUE_NAME;
import static com.niuma.huaapicommon.constant.RabbitMqConstant.SMS_QUEUE_NAME;
...
public class SendSmsListener {
    ...
    @RabbitListener(queues = SMS_QUEUE_NAME)
    public void sendSmsListenerByEmail(EmailDTO emailDTO, Message message, Channel channel) throws IOException {
        String messageId = message.getMessageProperties().getMessageId();
        int retryCount = (int) redisTemplate.opsForHash().get(RedisConstant.SMS_MESSAGE_PREFIX + messageId, "retryCount");
        if (retryCount > 3) {
            //重试次数大于3，直接放到死信队列
        ...
        try {
            ...
            if(StringUtils.isAnyBlank(emailNum,code)){
                throw new RuntimeException("sendSmsListener参数为空");
            }
            sendEmailUtils.sendMsg(emailNum, code);
            //手动确认消息
            channel.basicAck(message.getMessageProperties().getDeliveryTag(),false);
            log.info("短信发送成功：{}",emailDTO);
            redisTemplate.delete(RedisConstant.SMS_MESSAGE_PREFIX + messageId);
        } catch (Exception e) {
            redisTemplate.opsForHash().put(RedisConstant.SMS_MESSAGE_PREFIX+messageId,"retryCount",retryCount+1);
            channel.basicReject(message.getMessageProperties().getDeliveryTag(),true);
